/// https://github.com/FabianLars/tauri-plugin-deep-link/blob/main/src/windows.rs
///
/// Modified to accept multiple schemes.
use std::{
...
pub fn listen<F: FnMut(String) + Send + 'static>(mut handler: F) -> Result<()> {
    std::thread::spawn(move || {
        ...
        for conn in listener.incoming().filter_map(|c| {
            c.map_err(|error| error!("Incoming connection failed: {}", error))
                .ok()
        }) {
            ...
            if let Err(io_err) = conn.read_line(&mut buffer) {
                error!("Error reading incoming connection: {}", io_err.to_string());
            };
            buffer.pop();
...
pub fn prepare(identifier: &str) {
    if let Ok(mut conn) = LocalSocketStream::connect(identifier) {
        ...
        unsafe {
            ...
            if !success {
                warn!("AllowSetForegroundWindow failed.");
            }
        }
