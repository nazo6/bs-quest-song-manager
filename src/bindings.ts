...
async configGet() : Promise<__Result__<{ connection: ConnectionConfig | null }, string>> {
try {
...
async configSetConnection(connectionConfig: ConnectionConfig) : Promise<__Result__<null, string>> {
try {
    return { status: "ok", data: await TAURI_INVOKE("plugin:tauri-specta|config_set_connection", { connectionConfig }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type ConnectionConfig = { root: ModRoot; conn_type: ConnectionType }
export type ConnectionType = "Adb" | "Local"
