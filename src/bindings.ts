...
async cacheDumpGet(hash: string) : Promise<__Result__<{ Key: string; Hash: string; SongName: string; SongSubName: string; SongAuthorName: string; LevelAuthorName: string; Bpm: number; Upvotes: number; Downvotes: number; Duration: number }, string>> {
try {
    return { status: "ok", data: await TAURI_INVOKE("plugin:tauri-specta|cache_dump_get", { hash }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async cacheDumpUpdate() : Promise<__Result__<null, string>> {
try {
    return { status: "ok", data: await TAURI_INVOKE("plugin:tauri-specta|cache_dump_update") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
